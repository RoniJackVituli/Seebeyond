...
import { send_email_found_another_volunteer, send_email_to_client } from "../../../utils/functions/sendEmail";
...
class BlindController implements Controller {
  ...
  private INeedHelp = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        res.status(200).send({status:200,message:'ok', data:{volunteer:{
  ...
  private FindNextVolunteer = async (req: Request, res: Response, next: NextFunction) => {
    const {   email_volunteer, email_blind } = req.body;
    try {
        await this.delay(3000);
        const blind_user = await UserModel.findOne({ email: email_blind });
        const old_volunteer_user = await UserModel.findOne({email:email_volunteer});
        const new_volunteer_user = await UserModel.findOne({ type: 'volunteer', available: 'yes', email: { $ne: email_volunteer } }).sort({ rate: -1 });
        
        if(!new_volunteer_user || !blind_user || !old_volunteer_user) throw new HttpException(404, 'email not send');
  ...
  private rateVolunteer = async (req:Request, res:Response, next:NextFunction) => {
    try {
      ...
      const volunteer_user = await UserModel.findOne({email:volunteer.email});
      ...
      volunteer_user.blind = undefined,
